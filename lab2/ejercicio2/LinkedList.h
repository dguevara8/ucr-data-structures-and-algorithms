...
class LinkedList {
    ...
    public:
        ...
        // Se crea esta funcion como lo pide el enunciado    
        void removeDuplicates() {
            // LLama a la funcion que contiene head
            removeDuplicates2(head);
        }

        // Se crea la logica para tener un puntero al primer nodo de la lista enlazada sin tener que cambiar el main ya establecido
        void removeDuplicates2(Node *head) {
            // Si la lista está vacía, termina
            if (head == nullptr )
                return;

            // Almacena los valores únicos
            unordered_set<int> s;
            ...
            while (temp) {
                // Si el valor ya está en el set elimina el nodo
                if (s.count(temp->value)) {
                    prev->next = temp->next;
                    delete temp;
                    temp = prev->next;
                } else {
                    // Si es un nuevo valor lo guarda en el set
                    s.insert(temp->value);
                    prev = temp;
                    temp = temp->next;
                }
